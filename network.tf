...
resource "aws_security_group" "aws_windows_sg" {
  ...
  # SSH access from home office
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = [local.cidr_block_home_office]
  }

  # WinRM access from home office
  ingress {
    from_port   = 5985
    to_port     = 5985
    protocol    = "tcp"
  ...
  # Remote Desktop from home office
  ingress {
    from_port   = 3389
    to_port     = 3389
    protocol    = "tcp"
  ...
  # Remote Desktop from d home office
  ingress {
    from_port   = 3389
    to_port     = 3389
    protocol    = "tcp"
    cidr_blocks = ["24.134.116.21/32"]
  }
...
# ENI
resource "aws_network_interface" "lic_ser_eni" {
  subnet_id              = aws_subnet.public_subnets[0].id
  private_ips             = [cidrhost(aws_subnet.public_subnets[0].cidr_block, 5)]
  security_groups = [aws_security_group.aws_windows_sg.id]

  tags = {
    Name = "license-server-eni"
  }
}

resource "aws_eip" "lic_ser_eip" {
  network_interface = aws_network_interface.lic_ser_eni.id
}

resource "aws_eip" "webapp_ser_eip" {
